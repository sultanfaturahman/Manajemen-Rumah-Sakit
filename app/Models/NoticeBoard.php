...
class NoticeBoard extends Model
{
     ...
     * @return array
     */
    function prepareNoticeboardData(): array
    {
        return [
            'id'          => $this->id,
            'title'       => $this->title,
            'description' => $this->description,
            'date'        => isset($this->created_at) ? Carbon::parse($this->created_at)->format('d F y') : '',
            'time'        => isset($this->created_at) ? \Carbon\Carbon::parse($this->created_at)->isoFormat('LT') : '',
        ];
    }
}
